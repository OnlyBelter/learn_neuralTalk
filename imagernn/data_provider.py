...
class BasicDataProvider:
  def __init__(self, dataset):
    ...
    self.split = defaultdict(list)  # skills we can learn
  ...
  def _getImage(self, img):
    ...
    if not 'feat' in img: # also fill in the features
      ...
      img['feat'] = self.features[:,feature_index]  # add features from vgg_feats.mat
    return img
  ...
  def sampleImageSentencePair(self, split = 'train'):
    ...
    # random select a sentence from five sentences
    sent = random.choice(img['sentences'])

    out = {}
    out['image'] = self._getImage(img)  # img contains all info, i.e. an item of dataset['images']
    out['sentence'] = self._getSentence(sent)  # only dataset['images'][0]['sentence'] part
    return out
